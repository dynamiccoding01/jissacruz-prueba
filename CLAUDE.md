...
| [SPRINT6_UNIDADES_MEDIDAS_CODIGOS.md](SPRINT6_UNIDADES_MEDIDAS_CODIGOS.md) | **Sprint 6** (reunión del 26 jul 2026): unidades de medida, medidas, códigos originales, búsqueda sin acentos, rediseño del módulo Pedido y vigencia de proformas. Autosuficiente: decisiones ya cerradas con el cliente (Q1–Q32), riesgos (R8–R17) y checklist de avance. **Terminado en código y BD (scripts 22–30 corridos al 29 jul); lo que queda es operativo.** Si el trabajo toca alguno de esos temas, leerlo antes que nada |
| [TRD.md](TRD.md) | Stack definitivo, arquitectura, estructura de carpetas (§3), APIs internas |
| [BACKEND.md](BACKEND.md) | Esquema Postgres real, funciones RPC, políticas RLS, índices — **reescrito el 29 jul leyendo la base** (cubre hasta el script 30) |
...
**Server Components por defecto; Server Actions (`"use server"`, en `actions.ts` por módulo) para toda mutación; Route Handlers solo bajo `/api/pdf/*`** para generar PDFs con `@react-pdf/renderer` (no hay API REST pública). Cada módulo de `app/(dashboard)/<modulo>/` sigue el mismo patrón: `page.tsx` (Server Component, carga inicial de datos) + `<modulo>-explorer.tsx` (Client Component, tabla/filtros con `@tanstack/react-table`) + `<modulo>-form.tsx` (react-hook-form + zod) + `actions.ts` (Server Actions, valida con el schema de `lib/validations/<modulo>.ts` antes de tocar Supabase). Módulos existentes: `productos`, `inventario`, `kardex`, `proveedores`, `compras`, `clientes`, `proformas` (con detalle/edición en `proformas/[id]`), `ventas` (POS + historial), `traspasos` (el módulo "Pedido"), `sucursales`, `unidades-medida`, `reportes`, `dashboard`, `configuracion`.
...
Búsqueda avanzada de productos (código, equivalente, descripción, línea/marca, vehículo compatible) va por una sola RPC, `fn_buscar_productos(texto, campos text[])`, reutilizada por catálogo, compras, ventas/POS y proformas — no reimplementar el filtro en el cliente. El usuario elige por qué campos buscar (multi-selección) con el componente compartido [components/shared/criterios-busqueda.tsx](components/shared/criterios-busqueda.tsx); esos criterios (`campos`) se pasan a la RPC, que filtra solo por los marcados (OR entre ellos; arreglo vacío ⇒ busca en todos). Los `id` de los criterios en ese componente deben coincidir con los que evalúa el SQL. **La versión vigente de la función es la del `supabase/25_busqueda_original_medida.sql`**: suma los criterios `original` (códigos OEM) y `medida`, e incluye el `unaccent` del script 26 (buscar `valvula` encuentra `VÁLVULA`). Usa `EXISTS` para las tablas hijas, sin `LEFT JOIN` ni `DISTINCT`. ⚠️ Antes de reescribirla, confirmar qué está corriendo con `select prosrc from pg_proc where proname='fn_buscar_productos'` — el repo ya tuvo dos definiciones en conflicto.

**Auth y roles**: [lib/auth/session.ts](lib/auth/session.ts) expone `getPerfil()` (cacheado por request con `cache()` de React) y `requireAdmin()` para guardas de página. `app/(dashboard)/layout.tsx` ya valida sesión/`activo` y hace `signOut()` + redirect a `/login` si falla; las páginas exclusivas de admin deben llamar `requireAdmin()` igual, porque el layout no filtra por rol. Roles: `admin` (todo) y `vendedor` (solo Productos/Inventario en lectura, Proformas, Ventas/POS, Clientes — ver [components/shared/nav-items.ts](components/shared/nav-items.ts) para la matriz por ítem de navegación y FLUJO.md §10 para la matriz completa). RLS activo en todas las tablas; sin registro público, usuarios creados por invitación del admin desde Supabase Auth. Desde el script `30`, el **vendedor solo ve las ventas, proformas y pedidos de su sucursal** (política de `select` con `fn_es_admin() or ... = fn_mi_sucursal()`); catálogo, stock por sucursal y kardex los ven todos. **Ojo:** un perfil con `sucursal_id` en null deja de ver esos listados, sin error.
...
**Guardado de producto:** crear y editar producto pasan por la RPC transaccional `fn_guardar_producto` (script `29`, extendida por 22–24) — cabecera y todos los hijos (equivalentes, **originales OEM**, **medidas**, vehículos, precios por mayor) en una sola transacción. No volver al patrón de `delete` + `insert` sueltos desde la Server Action.

**Tres niveles de código de producto:** `productos.codigo` (el de la tienda) · `producto_codigos_originales` (OEM del fabricante, N por producto) · `producto_codigos_equivalentes` (de otro fabricante, N por producto). Ninguna de las dos tablas guarda `fabricante`. **Unidades de medida sin conversión:** un producto se vende solo en su unidad; el kardex y el FIFO no la miran.
...
**Precio de venta y costo:** `productos.precio` es el precio de **venta** y **no hay columna de costo** — el costo vive en los lotes del kardex y se deriva con [lib/costos-server.ts](lib/costos-server.ts). Regla de negocio (script `32`): **el precio de venta debe ser mayor al costo de compra**, exigida en el formulario, en la Server Action y por un `check` en `orden_compra_items`. El precio de venta se fija **al armar la orden de compra** (columna `precio_venta`) y `fn_recibir_orden_compra` lo aplica a `productos.precio` **al recibir**, no al crear la orden.
