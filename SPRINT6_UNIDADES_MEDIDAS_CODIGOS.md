...
| **Parte I** · Unidades / medidas / códigos originales | 🟨 **Fases 1 y 2 hechas · faltan correr scripts 22 y 23 · Fases 3–4 pendientes** | ✅ **Fase 1:** códigos originales (script `22`, migra los 810, quita `fabricante`). ✅ **Fase 2:** **medidas estructuradas** (script `23`: tabla `producto_medidas`, `fn_guardar_producto` con `p_medidas` → 7 args) + app (schema `medidas`, helper `lib/medidas.ts`, sección "Medidas" en el formulario con etiqueta/valor/unidad). `tsc`/`lint`/`build` limpios. ⚠️ FALTA **snapshot + correr el `22`, y luego el `23`** (en ese orden; hasta entonces guardar/ver un producto dará error). ⏳ **Fases 3–4 pendientes:** unidades de medida (`24`), búsqueda por original/medida + mostrar originales/medidas/unidad en listados (`25`). |
...
- [ ] ⏳ **Fase 3 (unidades) — script `24`** · **Fase 4 (búsqueda + mostrar en listados) — script `25`**. Pendientes.
