...
| Bloque | Estado | Notas |
|---|---|---|
| **R8** · guardado de producto sin transacción | ✅ **Hecho y verificado en la BD (27 jul)** | Mitigación mínima (dedup + chequeo de `delete`) **y** la **RPC transaccional `fn_guardar_producto`** (`supabase/29_fn_guardar_producto.sql`, **corrida en la BD**): crear/editar producto hace cabecera + reemplazo de hijos en UNA transacción atómica. `createProducto`/`updateProducto` la llaman (se eliminó `guardarHijos`). `tsc`/`lint`/`build` limpios. |
...
| **Parte I** · Unidades / medidas / códigos originales | 🟨 **Fases 1–3 hechas · faltan correr scripts 22, 23 y 24 · Fase 4 pendiente** | ✅ **Fase 1:** códigos originales (script `22`, migra los 810, quita `fabricante`). ✅ **Fase 2:** medidas estructuradas (script `23`). ✅ **Fase 3:** **catálogo de unidades de medida** (script `24`: tabla `unidades_medida` vacía + `productos.unidad_medida_id` FK + guarda R1 en `fn_guardar_producto`) + **ABM nuevo `/unidades-medida`** (admin, patrón `sucursales`) + ítem de nav "Unidades" + el formulario de producto usa un `<select>` de unidades (con respaldo al texto si aún no hay unidades) + etiquetas "Precio por unidad"/"Stock mínimo (en la unidad)" (R2). `tsc`/`lint`/`build` limpios. ⚠️ FALTA **snapshot + correr `22` → `23` → `24`** (en ese orden; hasta entonces guardar/ver un producto dará error). ⏳ **Fase 4 pendiente:** búsqueda por original/medida + mostrar originales/medidas/unidad en listados (`25`). |
...
- [ ] ⏳ **Fase 4 (búsqueda por original/medida + mostrar en listados) — script `25`**. Pendiente.
