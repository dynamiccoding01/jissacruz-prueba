...
| **Parte I** · Unidades / medidas / códigos originales | 🟨 **Fase 1 (códigos originales) hecha · falta correr script 22 · Fases 2–4 pendientes** | ✅ **Fase 1:** `supabase/22_codigos_originales.sql` (tabla `producto_codigos_originales`, migración de los 810, quita `fabricante`, reescribe `fn_guardar_producto` con `p_originales`) + app (schema `codigos_originales`, `fn_guardar_producto` con 6 args, formulario con sección "Códigos originales (OEM)" y sin input "Fabricante", `getProductoConDetalle` trae originales). `tsc`/`lint`/`build` limpios. ⚠️ FALTA **snapshot + correr el script `22`** (hasta entonces guardar/ver un producto dará error). ⏳ **Fases 2–4 pendientes:** medidas (`23`), unidades de medida (`24`), búsqueda por original/medida (`25`). |
...
- [ ] ⏳ **Fase 2 (medidas) — script `23`** · **Fase 3 (unidades) — script `24`** · **Fase 4 (búsqueda) — script `25`**. Pendientes.
