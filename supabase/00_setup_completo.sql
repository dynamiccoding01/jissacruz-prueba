  ...
  return query
    ...
    where p.activo
      and (
        ...
        where (
          ('codigo'      = any(v_campos) and p.codigo ilike '%' || tok || '%')
          or ('descripcion' = any(v_campos) and p.descripcion ilike '%' || tok || '%')
          or ('linea_marca' = any(v_campos) and p.linea_marca ilike '%' || tok || '%')
          or ('equivalente' = any(v_campos) and pce.codigo_equivalente ilike '%' || tok || '%')
          or ('vehiculo'    = any(v_campos) and (
                v.marca ilike '%' || tok || '%'
                or v.modelo ilike '%' || tok || '%'
             ))
        )
      )
...
-- ---------- Precios por mayor escalonados (C3) ----------
create table if not exists public.producto_precios_mayor (
  ...
  constraint producto_precios_mayor_pkey primary key (id)
);
...
declare
  ...
  v_base   numeric;
begin
  select precio into v_base from public.productos where id = p_producto_id;
  if v_base is null then return 0; end if;

  select precio into v_precio
  from public.producto_precios_mayor
  where producto_id = p_producto_id
  ...
  return coalesce(v_precio, v_base);
end;
$$;

revoke execute on function public.fn_obtener_precio_escalonado(uuid, integer, date) from public, anon;
grant  execute on function public.fn_obtener_precio_escalonado(uuid, integer, date) to authenticated;

-- ---------- Pedidos y Traspasos entre sucursales (C4) ----------
...
do $$
begin
  raise notice 'SISREP: setup completo OK (18 tablas + 1 vista, RPC, RLS, indices, storage).';
end;
$$;
