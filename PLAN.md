...
## Estado general — corte al 29 jul 2026
...
|---|---|---|
| **R8 · Guardado de producto sin transacción** — `updateProducto` borra los hijos (códigos equivalentes, vehículos, precios por mayor) y los reinserta sin transacción; si el insert falla, el producto queda sin ellos. | Alta | ✅ Hecho (28 jul) — RPC transaccional **`fn_guardar_producto`** (`supabase/29_fn_guardar_producto.sql`, extendida por los scripts 22–24 hasta 7 argumentos): cabecera + reemplazo de todos los hijos en **una sola transacción**. `createProducto`/`updateProducto` la llaman y se eliminó el `guardarHijos` que hacía varios INSERT/DELETE por HTTP. **Corrido y verificado en la BD.** |
...
| **Parte III-b · Stock en tránsito en el reporte de inventario** (H2) — con recorrido `origen → destino`, en pantalla + PDF + Excel. | Media | ✅ Hecho (28 jul) — bloque aparte en el reporte de inventario (no se suma al stock, porque la mercadería no está en ninguna sucursal), valorizado a costo FIFO del origen, en pantalla, PDF y hoja "En tránsito" del Excel. Sin SQL nuevo. De paso se corrigió el conteo de proformas, que daba `NaN` desde que apareció el estado `pendiente`. ⏳ Quedan **2 pedidos en `enviado`** por recibir desde la app (Q19, operativo). |
| **Parte I · Unidades de medida, medidas y códigos originales** — tabla `unidades_medida` (un producto se vende **solo** en su unidad, sin conversión), medidas múltiples por producto y tabla de códigos originales de fabricante, migrando los **810** códigos hoy mal clasificados como equivalentes con `fabricante = 'OEM'`. | Alta | ✅ Hecho y verificado en la BD (29 jul) — scripts `22`–`25`. **Fase 1:** `producto_codigos_originales` + migración de las **810** filas mal clasificadas + se elimina `fabricante` (verificado: 810 originales / 0 equivalentes / columna eliminada). **Fase 2:** `producto_medidas` (etiqueta, valor, unidad, orden) con helper `lib/medidas.ts` → `A: 45,40MM  B: 17,00MM`. **Fase 3:** catálogo `unidades_medida` + ABM `/unidades-medida` + `productos.unidad_medida_id` + guarda R1 (no cambiar la unidad de un producto con kardex). **Fase 4:** criterios `original` y `medida` en `fn_buscar_productos`, construidos **sobre la versión con `unaccent`** (verificado: la función viva tiene las dos cosas). Unidad, medidas y códigos OEM visibles en los buscadores del POS/proformas y en los PDF (tope de 5 códigos + "…"). ⏳ **Paso 3.4 pendiente:** asignar la unidad a los 239 productos y recién ahí eliminar la columna de texto. |
...
**Estado del entregable:** ✅ **Completo en código y base de datos (29 jul).** Los 9 scripts (`22`–`30`) corridos y verificados; `tsc` limpio y `build` compilando las 20 rutas; **ninguna pregunta abierta**. Falta solo lo operativo: el **paso 3.4** (asignar unidades a los 239 productos), recibir los **2 pedidos en `enviado`** y la verificación end-to-end de la Parte I en la UI.
...
- **Sprint 6 completo (29 jul 2026)** en código y base de datos: los 9 scripts (`22`–`30`) corridos y verificados. Falta solo lo operativo — el **paso 3.4** (asignar unidades a los 239 productos), recibir los **2 pedidos en `enviado`** y la verificación end-to-end de la Parte I en la UI. Detalle en [SPRINT6_UNIDADES_MEDIDAS_CODIGOS.md](SPRINT6_UNIDADES_MEDIDAS_CODIGOS.md).
- **`BACKEND.md` reescrito el 29 jul** leyendo el esquema real de la base (cubre hasta el script `30`): sucursales, stock por sucursal, pedidos, los tres niveles de código, medidas, unidades y la vigencia de proformas. `supabase/README.md` sigue siendo la referencia **operativa** (orden de ejecución y qué se corrió).
...
## Registro de cambios recientes — 30–31 jul 2026 (hallazgos de la prueba con el cliente)

Correcciones surgidas de probar el sistema con datos reales. **Los datos históricos no se tocaron** (decisión del cliente).

- 🔴 **BUG en el saldo del Kardex — `entrada_traspaso` se RESTABA en vez de sumarse.** [lib/kardex.ts](lib/kardex.ts) declaraba **4** tipos de movimiento cuando la BD tiene **6** desde el script 19; como el tipo mentía sobre los datos, `tsc` pasaba limpio y el error era invisible. Dos efectos: la columna "Movimiento" salía **vacía** para los traspasos y, sobre todo, **el saldo acumulado quedaba mal en 6 de los 17 productos con movimientos** — TKL20901 cerraba en **0** teniendo **100 unidades** reales. El mismo error salía por pantalla, PDF y Excel. **La BD siempre estuvo bien:** el saldo correcto coincide con `stock_actual` en los 6 casos; era solo presentación. Se corrigió con un conjunto explícito `TIPOS_ENTRADA` (agregar un tipo obliga a decidir de qué lado va, en vez de caer en "resta" por descarte) y se sumaron **columna Sucursal** y **signo en Cantidad**, porque un traspaso son dos filas con el mismo número y no se distinguía cuál entraba.
- 🔴 **Costos de compra inventados.** Al armar una orden, el campo "Costo Bs" arrancaba en 0 **sin ninguna referencia a la vista**, así que se cargaban números al azar; de ahí salieron productos comprados a Bs 1.220 que se vendían a Bs 564. Ahora el costo **se prellena con el último costo real** (derivado del kardex, [lib/costos-server.ts](lib/costos-server.ts)) y los resultados de búsqueda muestran **precio de venta y último costo**.
- ✅ **Regla de precios: el precio de venta debe superar el costo de compra** (`supabase/32_precio_venta_en_compra.sql`, corrido y verificado el 31 jul). La orden de compra suma una columna **Precio venta Bs** por ítem, prellenada con el precio actual. Se valida en **tres capas**: formulario (campo en rojo + botón deshabilitado), Server Action (zod) y **`check` en la BD**. `fn_recibir_orden_compra` **aplica ese precio a `productos.precio` al recibir** — no al crear la orden, porque una orden pendiente se puede cancelar y la mercadería todavía no existe. Además, la ficha del producto **rechaza guardar** un precio que no supere el último costo, y muestra ese costo como referencia mientras se edita.
- ✅ **Carritos de Pedido y Orden de compra** al mismo nivel que proforma: modal ancho, los 7 criterios de búsqueda (Pedido no tenía ninguno) y **Enter agrega el primer resultado**.
- ✅ **Historial de ventas solo para el admin**: el vendedor ve únicamente el POS y la página ni siquiera hace esa consulta.
- 📌 **Decisión del cliente:** los datos ya registrados con costos inventados **se dejan como están** (lotes FIFO de TKL542013010, 457 y TKL35014). El margen de esas ventas queda distorsionado hasta agotar los lotes.
- ⏳ **Pendiente de precios:** tras la carga del cliente quedan **9 productos** con compras y precio 0 que **no se pueden guardar** hasta precificarlos, y **231 de 239** siguen en precio 0 a la espera de la lista completa.

---

## Registro de cambios recientes — 28–29 jul 2026 (Sprint 6 completo + RLS por sucursal)

Segunda tanda del **Sprint 6**: se cerró todo lo que faltaba y, de yapa, el **paso 4 (B) del Sprint 5 · C2**. Los 9 scripts (`22`–`30`) están corridos y **verificados contra la BD real**, no solo commiteados.

- ✅ **R8 cerrado del todo** — RPC transaccional `fn_guardar_producto` (script `29`, extendida por el 22/23/24 hasta 7 argumentos). Crear o editar un producto pasa a ser **una sola transacción**: si algo falla, Postgres revierte todo. Se eliminó el `guardarHijos` que hacía varios INSERT/DELETE por HTTP sin transacción.
- ✅ **Parte I completa** (scripts `22`–`25`). Migración verificada: **810** códigos originales, **0** equivalentes, columna `fabricante` eliminada. Módulo nuevo `/unidades-medida` (ABM admin) y `productos.unidad_medida_id`. **Sin conversión de unidades**, según la decisión del cliente: la unidad es un atributo del producto y el kardex/FIFO no se tocan.
- ✅ **El riesgo del script 25 se sorteó bien.** Los scripts 25 y 26 reescriben la **misma** función y el 25 podía revertir el arreglo de acentos sin que nadie lo notara. Se construyó sobre la versión viva: la `fn_buscar_productos` que corre tiene **`unaccent` Y `producto_medidas`**. Probado en la base: `valvula` sin tilde devuelve **113** productos y la búsqueda por código OEM con criterio `original` encuentra el suyo. Además quedó **una sola** versión de cada función (5→6→7 argumentos con `drop` en cada paso), así que **no hay ambigüedad de PostgREST**.
- ✅ **Parte III-b** — stock en tránsito en el reporte de inventario, como bloque aparte (la mercadería despachada y no recibida **no está en ninguna sucursal**), valorizado a costo FIFO del origen, en pantalla, PDF y Excel. De paso se corrigió el conteo de proformas, que daba `NaN` desde que apareció el estado `pendiente`.
- ✅ **R15, R16 y R17 corregidos.** R15 se resolvió invirtiendo el orden en `updateProforma`: ítems primero, cabecera y `revalidada_en` al final. **Residuo de R16:** las 3 proformas ya convertidas quedaron con el plazo pisado a 3 días desde la primera corrida del script 27; el fix evita que se repita pero no revierte (datos de prueba).
- ✅ **Q6 cerrada** (`TOPE_OEM = 5` en los PDF de proforma y venta). **Ya no queda ninguna pregunta abierta del Sprint 6.**
- ✅ **Sprint 5 · C2 paso 4 (B): RLS por sucursal para el vendedor** (script `30`). El vendedor ve solo ventas, proformas y pedidos de **su** sucursal; el admin ve todo. El catálogo, el stock por sucursal y el kardex **no** se restringen, para poder decidir traspasos. **Parte A (eliminar `productos.stock_actual`) queda como deuda aceptada:** la columna la mantiene un trigger, es correcta, y quitarla es alto riesgo y bajo valor.
- ✅ **RESUELTO (29 jul): la RLS por sucursal se verificó con vendedores reales** y se cerró su hueco. Simulando la sesión en SQL (`set local role authenticated` + `request.jwt.claims`, en transacción con `rollback`): el vendedor de **Casa Matriz** ve 12 ventas / 5 proformas, el de **Almacén Centro** 2 / 0, el **admin** las 14 / 5; el catálogo (239 productos) lo ven todos y los proveedores solo el admin. Los 10 pedidos los ven los dos vendedores porque **solo hay 2 sucursales** y todo traspaso es 1↔2. **El modo de fallo silencioso era real:** con `perfiles.sucursal_id` en null el vendedor veía 0 ventas, 0 proformas y 0 pedidos, sin error, pero sí los 239 productos — la app parecía sana. Lo cierra el **script `31_perfil_sucursal_obligatoria.sql`**: el trigger de alta resuelve una sucursal por defecto (y falla con mensaje claro si no hay ninguna activa), backfill de los null y `not null` en la columna. Probado antes con `rollback` y después corrido y verificado.
- ⏳ **Otros pendientes del sprint (operativos, no de código):** paso 3.4 de la Parte I (hay 2 unidades cargadas pero los **239 productos siguen con `unidad_medida_id` en null**, y la columna de texto todavía convive con la FK); recibir los **2 pedidos en `enviado`**; verificación end-to-end de la Parte I en la UI.
...
- 📝 **Documentación puesta al día:** `supabase/README.md` con los scripts 26–28 marcados como corridos, y esta sección + el bloque "SPRINT 6" arriba. *(`BACKEND.md` se reescribió después, el 29 jul.)*
...
- 🔴 **BUG CRÍTICO encontrado y corregido — recibir un traspaso RESTABA stock en el destino**: `fn_kardex_aplica_stock` (script 14) solo trataba como entrada `entrada_compra`/`ajuste_entrada`; el script 19 agregó `entrada_traspaso` sin actualizar el trigger, así que la recepción caía en la rama "else" (resta) y el producto "salía" de ambas sucursales. **Fix: `supabase/20_fix_trigger_traspasos.sql`** — corrige el trigger, **recomputa el cache de stock desde el kardex** (repara recepciones ya hechas con el bug) y de paso agrega el `UNIQUE (producto_id, cantidad_minima)` que le faltaba a `producto_precios_mayor` en la BD real. ✅ **Corrido y verificado en la BD real el 20 jul** (verificación `trigger_corregido = true`). Módulo `/traspasos` seguro de usar.
- ✅ **Build de producción (`npm run build`) pasa completo**: las 14 rutas compilan (incluida `/traspasos`), bundles razonables (First Load compartido ~88 KB) — por el lado del código el sistema **está listo para Vercel**.
- ✅ Esquema real de la BD verificado contra la app: migraciones 11–19 aplicadas (sucursales, stock por sucursal, documentos con `sucursal_id`, `tiempo_entrega_dias`, precios por mayor, traspasos con tipos de kardex y `referencia_tipo` `traspaso`). `fn_enviar_traspaso` consume FIFO del origen correctamente; RLS de traspasos solo-select + escritura vía RPC `security definer` (mismo criterio que ventas).
- 📝 Deuda conocida que NO bloquea el despliegue: `00_setup_completo.sql` sigue sin incorporar los scripts 12–14, 16 y 20 (solo afecta instalaciones desde cero); C2 paso 4 pendiente (total repetido + RLS por sucursal del vendedor); precio = 0 en los 236 productos TKL hasta que el cliente pase la lista.
